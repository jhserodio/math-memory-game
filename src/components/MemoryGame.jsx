...
const MemoryGame = () => {
  ...
  const { size, attempt, cardCount, cards } = useSelector(
  ...
  const check = (index, updatedData) => {
    ...
    if (updatedChecklist.length >= 2) {
      setTimeout(() => {
        ...
        const first = updatedData[updatedChecklist[0]];
        const second = updatedData[updatedChecklist[1]];
  ...
  const cardsList = gameCards.map((item, index) => {
    return (
      <Card
        key={`${index}-card`}
        data={item}
  ...
  return (
    <div className={`${style.MemoryGame} game w-100`}>
      {alert && show && (
        <Alert
      ...
      <div className={`${style.section} container`}>
        <div className="row content flex-wrap">
          <div className={`${style.header} col-md-7`}>
            <Header />
            <Attempts currentAttempt={currentAttempt} maxAttempt={attempt} />
            <div className="row main-game">{cardsList}</div>
          </div>
